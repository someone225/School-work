...
def main():
    ...
    new_img = image()
    new_img.in_path = str(input("Enter the path of the image you want to load: "))
    load_img(new_img.in_path)
...
class image:
    ...
    def __init__(self):
        t = Image.open(self.in_path) #PIL.image.open
        self.img_data = np.asarray(t, dtype= np.uint8) #image array
